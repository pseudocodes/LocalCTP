...
CLeeDateTime::CLeeDateTime(const struct tm& _tm)
{
    ...
    if (_tm.tm_year < 70 || (_tm.tm_year == 70 && _tm.tm_mon == 0 && _tm.tm_mday == 1 && _tm.tm_hour < time_zone)) // 在1970/1/1(再加上时区)之前的struct tm日期, 通过mktime(&_tm)转换为time_t时, 得到-1
    {
        // 每个月的天数. 非闰年
        ...
        time_t total_second_num = rest_day_num_of_this_year_to_1970 * 24LL * 60LL * 60LL
            ...
            - _tm.tm_min * 60LL
            - _tm.tm_sec; // 若time_t为32位,则可能溢出. 使用24LL而不是24等.
...
CLeeDateTime::CLeeDateTime(const ::SYSTEMTIME& st)
{
    ...
    /*long long ll =
            ((long long)fileTime1.dwHighDateTime << 32) | fileTime1.dwLowDateTime;*/
    // 比较慢
...
{
#ifdef _WIN32
    ::SYSTEMTIME sysTime1 = CLeeDateTime(m_dt + HALF_SECOND).Get_SYSTEMTIME(); // 因为tm结构体没有毫秒字段,所以加上半秒以四舍五入.
...
{
#ifdef _WIN32
    ::SYSTEMTIME sysTime1 = CLeeDateTime(m_dt - CLeeDateTime::GetTimeZone() / 24.0 + HALF_SECOND).Get_SYSTEMTIME(); // 因为tm结构体没有毫秒字段,所以加上半秒以四舍五入.
    struct tm newtime {
        static_cast<int>(sysTime1.wSecond),
        static_cast<int>(sysTime1.wMinute),
        static_cast<int>(sysTime1.wHour),
...
bool CLeeDateTime::operator==(const CLeeDateTime& rhs) const
{
    return m_dt - rhs.m_dt < CLeeDateTime::THREE_MILLISECOND && rhs.m_dt - m_dt < CLeeDateTime::THREE_MILLISECOND;
}
