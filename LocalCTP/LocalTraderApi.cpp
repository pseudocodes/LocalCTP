...
#define CHECK_LOGIN(p, memberName)                              \
    ...
    if (!m_logined)                                             \
        return -1;                                              \
    if (m_userID != p->memberName || m_brokerID != p->BrokerID) \
        return -1;
...
CSettlementHandler& CLocalTraderApi::settlementHandler = CSettlementHandler::getSettlementHandler(CLocalTraderApi::sqlHandler);
...
CSqliteHandler CLocalTraderApi::sqlHandler("LocalCTP.db", { "CThostFtdcInvestorPositionField", "CThostFtdcInvestorPositionDetailField", "CThostFtdcOrderField", "CThostFtdcTradeField", "CThostFtdcTradingAccountField", "CThostFtdcInstrumentField", "CThostFtdcInstrumentMarginRateField", "CThostFtdcInstrumentCommissionRateField", "CloseDetail", "SettlementData" });
const long long CLocalTraderApi::initStartTime = CLeeDateTime::GetCurrentTime().Get_time_t() * 1000 + CLeeDateTime::GetCurrentTime().GetMillisecond(); // 1612345678 999
    ...
    const MarketDataVec& mdVec, TradePriceVec& tradePriceVec)
{
    ...
    } else // 单个独立的合约,或组合合约
    {
        ...
        for (std::size_t legNo = 0; legNo < mdVec.size(); ++legNo) {
            auto directionT = (legNo % 2 == 0 ? direction : getOppositeDirection(direction));
        ...
        return ((direction == THOST_FTDC_D_Buy && GE(orderPrice, priceDiff)) || (direction == THOST_FTDC_D_Sell && LE(orderPrice, priceDiff)));
    }
}
...
CLocalTraderApi::CLocalTraderApi(const char* pszFlowPath /* = ""*/)
    ...
    , m_logined(false)
    , m_frontID(static_cast<int>(CLeeDateTime::GetCurrentTime().Get_time_t()))
    , m_sessionID(0)
    , m_tradingAccount { 0 }
    , m_messageQueue()
    , m_mdReceiver(pszFlowPath, this)
    , m_successRspInfo { 0, "success" }
    , m_errorRspInfo { -1, "error" }
{
...
void CLocalTraderApi::onSnapshot(const CThostFtdcDepthMarketDataField& mdData)
{
    ...
    }
    {
        ...
        if (std::stoi(updateTimeStr.substr(0, 2)) >= 20) // 夜盘行情时间
        {
            ...
            if (tradingDay.empty() || minDayStr.size() < 8)
                return;
...

    {
        ...
        for (auto& sessionOrders : m_orderData) {
            for (auto& o : sessionOrders.second) {
                ...
                if (std::string(order.rtnOrder.InstrumentID).find(instrumentID) == std::string::npos || order.isDone()) {
                    continue;
                }
                MarketDataVec mdVec;
    ...
    const double priceForCal = isPriceValid(mdData.SettlementPrice) ? mdData.SettlementPrice : mdData.LastPrice; // 优先使用结算价进行计算
            ...
            if (itPos != m_positionData.end()) {
                for (auto& posDetail : itPos->second.posDetailData) {
                    ...
                    if (isOptions(it->second.ProductClass)) {
                        ...
                        const double positionCostOfThiPosDetail = (strcmp(posDetail.OpenDate, posDetail.TradingDay) == 0 ? posDetail.OpenPrice : posDetail.LastSettlementPrice)
                            * posDetail.Volume * it->second.VolumeMultiple;
                        const double openCostOfThiPosDetail = posDetail.OpenPrice * posDetail.Volume * it->second.VolumeMultiple;
                        posDetail.PositionProfitByTrade = (dir == THOST_FTDC_D_Buy ? 1 : -1) * (priceForCal * it->second.VolumeMultiple * posDetail.Volume - openCostOfThiPosDetail);
                        posDetail.PositionProfitByDate = (dir == THOST_FTDC_D_Buy ? 1 : -1) * (priceForCal * it->second.VolumeMultiple * posDetail.Volume - positionCostOfThiPosDetail);
                    }

                    // only update some fields in position detail table
                    const std::string updatePositionDetailProfitSql = std::string("UPDATE 'CThostFtdcInvestorPositionDetailField' SET PositionProfitByTrade=")
                ...
                if (isOptions(it->second.ProductClass)) {
                    ...
                    PositionProfit = (dir == THOST_FTDC_D_Buy ? 1 : -1) * (priceForCal * it->second.VolumeMultiple * itPos->second.pos.Position - itPos->second.pos.PositionCost);
                    diffPositionProfit += PositionProfit - oldPositionProfit;
                }

                // only update some fields in position table
                const std::string updatePositionProfitSql = std::string("UPDATE 'CThostFtdcInvestorPositionField' SET PositionProfit=")
...
{
    if (THOST_FTDC_OST_Canceled != o.OrderStatus)
        return;
    ...
    auto handleOpen = [&]() {
        ...
        if (isOptions(it->second.ProductClass)) {
            ...
            {
                frozenCash = itDepthMarketData->second.PreSettlementPrice * it->second.VolumeMultiple * o.VolumeTotal;
            } else // 期权卖出的(开仓)委托, 需解冻一部分保证金(期权保证金计算太复杂啦,本系统以期货保证金计算规则来算期权保证金)
            {
            }
        }
        frozenMargin = itDepthMarketData->second.PreSettlementPrice * it->second.VolumeMultiple * o.VolumeTotal * itPos->second.pos.MarginRateByMoney
    ...
    auto handleClose = [&]() {
        ...
        int& frozenPositionNum = (itPos->second.pos.PosiDirection == THOST_FTDC_PD_Long ? itPos->second.pos.ShortFrozen : itPos->second.pos.LongFrozen);
    ...
    if (isOpen(o.CombOffsetFlag[0])) {
        ...
        if (it->second.ProductClass == THOST_FTDC_PC_Combination) {
            ...
            for (std::size_t legNo = 0; legNo < singleContracts.size(); ++legNo) {
                ...
                directionT = (legNo % 2 == 0 ? o.Direction
                                             : getOppositeDirection(o.Direction));

                ((o.IsSwapOrder != 0 && legNo % 2 != 0) ? handleClose() : handleOpen()); // 需考虑互换单的情况
            }
        } else {
            handleOpen();
        }
    } else // 平仓报单已撤单,则减少冻结持仓(对应数量是未成交数量)
    {
        ...
        if (it->second.ProductClass == THOST_FTDC_PC_Combination) {
            ...
            for (std::size_t legNo = 0; legNo < singleContracts.size(); ++legNo) {
                instr = singleContracts[legNo];
                directionT = (legNo % 2 == 0 ? getOppositeDirection(o.Direction)
                                             : o.Direction);

                ((o.IsSwapOrder != 0 && legNo % 2 != 0) ? handleOpen() : handleClose()); // 需考虑互换单的情况
            }
        } else {
            handleClose();
        }
...
void CLocalTraderApi::updateByTrade(const CThostFtdcTradeFieldWrapper& t)
{
    ...
    if (isOptions(it->second.ProductClass)) {
        ...
        {
            frozenCash = itDepthMarketData->second.PreSettlementPrice * it->second.VolumeMultiple * pTrade->Volume;
    ...
    if (isOpen(pTrade->OffsetFlag)) {
        const double feeOfTrade = pTrade->Price * it->second.VolumeMultiple * pTrade->Volume * itCommissionRate->second.OpenRatioByMoney + pTrade->Volume * itCommissionRate->second.OpenRatioByVolume;
        ...
        const double frozenMargin = itDepthMarketData->second.PreSettlementPrice * it->second.VolumeMultiple * pTrade->Volume * itPos->second.pos.MarginRateByMoney
            + pTrade->Volume * itPos->second.pos.MarginRateByVolume;
        const double marginOfTrade = pTrade->Price * it->second.VolumeMultiple * pTrade->Volume * itPos->second.pos.MarginRateByMoney
            + pTrade->Volume * itPos->second.pos.MarginRateByVolume;
        {
            ...
            pos.PositionCost += pTrade->Price * pTrade->Volume * itPos->second.volumeMultiple; // 更新持仓的持仓成本
            pos.OpenVolume += pTrade->Volume;
            pos.OpenAmount += pTrade->Volume * pTrade->Price * itPos->second.volumeMultiple;
            pos.OpenCost += pTrade->Volume * pTrade->Price * itPos->second.volumeMultiple;
    ...
    // 平仓成交时,按"先开先平"的原则更新持仓明细,减少持仓中的冻结持仓,更新持仓和资金.
        ...
        if (itPos == m_positionData.end()) {
            ...
            for (auto& p : itPos->second.posDetailData) {
                ...
                p.CloseAmount += tradeVolumeInThisPosDetail * pTrade->Price * itPos->second.volumeMultiple;
                const double positionCostOfThiPosDetail = (strcmp(p.OpenDate, p.TradingDay) == 0 ? p.OpenPrice : p.LastSettlementPrice)
                    * p.Volume * it->second.VolumeMultiple;
                if (!isOptions(it->second.ProductClass)) // 期权无持仓盈亏
                {
                    const double openCostOfThiPosDetail = p.OpenPrice * p.Volume * it->second.VolumeMultiple;
                    p.PositionProfitByTrade = (p.Direction == THOST_FTDC_D_Buy ? 1 : -1) * (pTrade->Price * it->second.VolumeMultiple * p.Volume - openCostOfThiPosDetail); // 更新持仓明细的逐笔对冲持仓盈亏
                    p.PositionProfitByDate = (p.Direction == THOST_FTDC_D_Buy ? 1 : -1) * (pTrade->Price * it->second.VolumeMultiple * p.Volume - positionCostOfThiPosDetail); // 更新持仓明细的逐日盯市持仓盈亏
                }
                ...
                if (strcmp(p.OpenDate, p.TradingDay) != 0) // 昨仓
                {
                    ...
                    {
                        closeProfitOfThisPosDetailInThisTrade = (pos.PosiDirection == THOST_FTDC_PD_Long ? 1 : -1) * (pTrade->Price - pos.PreSettlementPrice) * tradeVolumeInThisPosDetail * itPos->second.volumeMultiple;
                    }
                ...
                } else // 今仓
                {
                    ...
                    if (!isOptions(it->second.ProductClass)) // 期权无平仓盈亏
                    {
                        closeProfitOfThisPosDetailInThisTrade = (pos.PosiDirection == THOST_FTDC_PD_Long ? 1 : -1) * (pTrade->Price - p.OpenPrice) * tradeVolumeInThisPosDetail * itPos->second.volumeMultiple;
                    }
                    _closeFlag = THOST_FTDC_OF_CloseToday;
                }
                if (!isOptions(it->second.ProductClass)) // 期权无平仓盈亏
                {
                    double closeProfitByTradeOfThisPosDetailInThisTrade = (pos.PosiDirection == THOST_FTDC_PD_Long ? 1 : -1) * (pTrade->Price - p.OpenPrice) * tradeVolumeInThisPosDetail * itPos->second.volumeMultiple;
            ...
            double feeCloseYesterdayOfTrade = pTrade->Price * it->second.VolumeMultiple * closeYesterdayVolume * itCommissionRate->second.CloseRatioByMoney + closeYesterdayVolume * itCommissionRate->second.CloseRatioByVolume;
            double feeCloseTodayOfTrade = pTrade->Price * it->second.VolumeMultiple * closeTodayVolume * itCommissionRate->second.CloseTodayRatioByMoney + closeTodayVolume * itCommissionRate->second.CloseTodayRatioByVolume;
            ...
            for (auto& p : itPos->second.posDetailData) {
                ...
                pos.PositionCost += p.Volume * (strcmp(p.OpenDate, p.TradingDay) == 0 ? p.OpenPrice : pos.PreSettlementPrice)
                    * itPos->second.volumeMultiple; // 持仓明细的持仓成本汇总计算中,昨仓用昨结算价,今仓用开仓价
                pos.OpenCost += p.Volume * p.OpenPrice * itPos->second.volumeMultiple;
            }
            if (!isOptions(it->second.ProductClass)) // 期权无持仓盈亏
            {
                pos.PositionProfit = (pos.PosiDirection == THOST_FTDC_PD_Long ? 1 : -1)
            ...
            int& frozenPositionNum = (pos.PosiDirection == THOST_FTDC_PD_Long ? pos.ShortFrozen : pos.LongFrozen);
            frozenPositionNum = (std::max)(
            ...
            pos.CloseAmount += pTrade->Volume * pTrade->Price * itPos->second.volumeMultiple; // 更新持仓的平仓金额
        ...
        for (const auto& rowData : posSqlValues) {
            ...
            for (const auto& posDetail : posDetails) {
                auto posDetailMatchPos = [&]() -> bool {
                    bool isMatch = strcmp(posDetail.InstrumentID, posData.pos.InstrumentID) == 0 && strcmp(posDetail.ExchangeID, posData.pos.ExchangeID) == 0 && ((posDetail.Direction == THOST_FTDC_D_Buy && posData.pos.PosiDirection == THOST_FTDC_PD_Long) || (posDetail.Direction == THOST_FTDC_D_Sell && posData.pos.PosiDirection == THOST_FTDC_PD_Short));
                    if (!isMatch)
                        return isMatch;
    ...
    auto generateOrderData = [&]() {
        ...
        for (auto& order : ordersInDb) {
            ...
            for (auto& trade : tradesInDb) {
                if (strcmp(trade.data.OrderSysID, order.OrderSysID) == 0 && strcmp(trade.data.ExchangeID, order.ExchangeID) == 0) {
...
CLeeDateTime CLocalTraderApi::getNowTime()
{
    ...
    case RUNNING_MODE::BACKTEST_MODE:
        return (m_latestMarketTime == CLeeDateTime() ? m_defaultTimeInBackTestMode : m_latestMarketTime);
...
{
    if (pReqAuthenticateField == nullptr || !m_bRunning)
        return -1;
    ...
    if (strlen(pReqAuthenticateField->UserID) == 0 || strlen(pReqAuthenticateField->BrokerID) == 0) {
    ...
    if ((!m_userID.empty() && m_userID != pReqAuthenticateField->UserID) || (!m_brokerID.empty() && m_brokerID != pReqAuthenticateField->BrokerID)) {
    ...
    std::string relativeOrderSysID /*= std::string()*/)
{
    ...
    if (pInputOrder->ContingentCondition != THOST_FTDC_CC_Immediately && !isConditionalType(pInputOrder->ContingentCondition)) {
        sendRejectOrder(ErrMsg_NotSupportContingentCondition);
        return 0;
    }

    if (pInputOrder->TimeCondition != THOST_FTDC_TC_GFD && pInputOrder->TimeCondition != THOST_FTDC_TC_IOC) {
    ...
    auto handleOpen = [&](bool preCheck) -> std::pair<bool, std::string> {
        ...
        double MarginRatioByMoney = (directionT == THOST_FTDC_D_Buy ? itMarginRate->second.LongMarginRatioByMoney
                                                                    : itMarginRate->second.ShortMarginRatioByMoney);
        double MarginRatioByVolume = (directionT == THOST_FTDC_D_Buy ? itMarginRate->second.LongMarginRatioByVolume
        ...
        if (isOptions(itInstr->second.ProductClass)) {
            ...
            {
                frozenCash = itDepthMarketData->second.PreSettlementPrice * itInstr->second.VolumeMultiple * orderNum;
        ...
        const double frozenMargin = itDepthMarketData->second.PreSettlementPrice * itInstr->second.VolumeMultiple * orderNum * MarginRatioByMoney
    ...
    auto handleClose = [&](bool preCheck) -> std::pair<bool, std::string> {
        ...
        PositionDataMap::iterator itPos;
        {
            ...
            itPos = m_positionData.find(posKey);
            if (itPos == m_positionData.end()) {
                return std::make_pair(false, ERRMSG_AVAILABLE_POSITION_NOT_ENOUGH + std::string("0") + " on " + instr);
            }
        }
        int& frozenPositionNum = (itPos->second.pos.PosiDirection == THOST_FTDC_PD_Long ? itPos->second.pos.ShortFrozen : itPos->second.pos.LongFrozen);
        ...
        if (closable < orderNum) {
            return std::make_pair(false,
                ((isSpecialExchange(itInstr->second.ExchangeID) && pInputOrder->CombOffsetFlag[0] == THOST_FTDC_OF_CloseToday) ? ERRMSG_AVAILABLE_TODAY_POSITION_NOT_ENOUGH : ERRMSG_AVAILABLE_POSITION_NOT_ENOUGH)
        ...
        if (isOpen(pInputOrder->CombOffsetFlag[0])) {
            if (itInstr->second.ProductClass == THOST_FTDC_PC_Combination) {
                ...
                for (std::size_t legNo = 0; legNo < singleContracts.size(); ++legNo) {
                    ...
                    auto handleSingleContractRet = ((pInputOrder->IsSwapOrder != 0 && legNo % 2 != 0) ? handleClose(preCheck) : handleOpen(preCheck)); // 需考虑互换单的情况
            ...
            if (itInstr->second.ProductClass == THOST_FTDC_PC_Combination) {
                ...
                for (std::size_t legNo = 0; legNo < singleContracts.size(); ++legNo) {
                    ...
                    auto handleSingleContractRet = ((pInputOrder->IsSwapOrder != 0 && legNo % 2 != 0) ? handleOpen(preCheck) : handleClose(preCheck)); // 需考虑互换单的情况
                    if (!handleSingleContractRet.first) {
                        return handleSingleContractRet;
                    }
                }
    ...
    int OrderRef = atoi(pInputOrderAction->OrderRef);
    {
        ...
        if (it != m_orderData.end()) {
            ...
            if (itOrder != it->second.end()) {
                ...
                if (order.isDone() || order.rtnOrder.FrontID != pInputOrderAction->FrontID || order.rtnOrder.SessionID != pInputOrderAction->SessionID || strcmp(order.rtnOrder.InstrumentID, pInputOrderAction->InstrumentID) != 0) {
                    m_messageQueue.addMsg(OnRspOrderActionMsg(pInputOrderAction, setErrorMsgAndGetRspInfo(order.isDone() ? ErrMsg_AlreadyDoneOrder : ErrMsg_NotExistOrder),
                        nRequestID, true));
                    return 0;
                } else {
                    order.handleCancel();
        ...
        for (auto& sessionOrderPair : m_orderData) {
            for (auto& orderPair : sessionOrderPair.second) {
                ...
                if (strcmp(order.rtnOrder.OrderSysID, pInputOrderAction->OrderSysID) == 0 && strcmp(order.rtnOrder.ExchangeID, pInputOrderAction->ExchangeID) == 0) {
                    order.handleCancel();
                    return 0;
                }
            }
...
int CLocalTraderApi::ReqSettlementInfoConfirm(CThostFtdcSettlementInfoConfirmField* pSettlementInfoConfirm, int nRequestID)
{
    ...
    // 更新最新的交易日的结算结果确认信息
    const std::string UPDATE_NEWEST_SETTLEMENT_RECORD_TO_CONFIRMED = std::string("UPDATE 'SettlementData' SET ConfirmDay='") + GetTradingDay()
    ...
    std::vector<CThostFtdcOrderField*> v;
    {
        ...
        for (auto& sessionOrders : m_orderData) {
            for (auto& o : sessionOrders.second) {
                ...
                if (COMPARE_MEMBER_MATCH(pQryOrder, rtnOrder, ExchangeID) && COMPARE_MEMBER_MATCH(pQryOrder, rtnOrder, OrderSysID) && COMPARE_MEMBER_MATCH(pQryOrder, rtnOrder, InstrumentID)) {
        ...
        for (auto& sessionOrders : m_orderData) {
            for (auto& o : sessionOrders.second) {
                for (auto& t : o.second.rtnTrades) {
                    if (COMPARE_MEMBER_MATCH(pQryTrade, t.data, ExchangeID) && COMPARE_MEMBER_MATCH(pQryTrade, t.data, TradeID) && COMPARE_MEMBER_MATCH(pQryTrade, t.data, InstrumentID)) {
...
int CLocalTraderApi::ReqQryInvestorPosition(CThostFtdcQryInvestorPositionField* pQryInvestorPosition, int nRequestID)
{
    ...
    std::vector<CThostFtdcInvestorPositionField*> v;
    {
        ...
        for (auto& o : m_positionData) {
            ...
            if (COMPARE_MEMBER_MATCH(pQryInvestorPosition, pos, ExchangeID) && COMPARE_MEMBER_MATCH(pQryInvestorPosition, pos, InstrumentID)) {
                v.emplace_back(&pos);
            }
        }
    }
...
{
    if (pQryInstrumentMarginRate == nullptr || !m_logined)
        return -1;
    ...
    for (auto& instrPair : m_instrumentMarginRateData) {
        ...
        if (COMPARE_MEMBER_MATCH(pQryInstrumentMarginRate, marginRate, ExchangeID) && COMPARE_MEMBER_MATCH(pQryInstrumentMarginRate, marginRate, InstrumentID)) {
...
{
    if (pQryInstrumentCommissionRate == nullptr || !m_logined)
        return -1;
    ...
    for (auto& instrPair : m_instrumentCommissionRateData) {
        ...
        if (COMPARE_MEMBER_MATCH(pQryInstrumentCommissionRate, commissionRate, ExchangeID) && COMPARE_MEMBER_MATCH(pQryInstrumentCommissionRate, commissionRate, InstrumentID)) {
...
{
    if (pQryExchange == nullptr || !m_logined)
        return -1;
...
{
    if (pQryProduct == nullptr || !m_logined)
        return -1;
    std::vector<CThostFtdcProductField*> v;
    for (auto& productPair : m_products) {
        auto& product = productPair.second;
        if (COMPARE_MEMBER_MATCH(pQryProduct, product, ExchangeID) && COMPARE_MEMBER_MATCH(pQryProduct, product, ProductID) && pQryProduct->ProductClass == product.ProductClass) {
...
{
    if (pQryInstrument == nullptr || !m_logined)
        return -1;
    ...
    for (auto& instrPair : m_instrData) {
        ...
        if (COMPARE_MEMBER_MATCH(pQryInstrument, instr, ExchangeID) && COMPARE_MEMBER_MATCH(pQryInstrument, instr, ProductID) && COMPARE_MEMBER_MATCH(pQryInstrument, instr, InstrumentID)) {
...
{
    if (pQryDepthMarketData == nullptr || !m_logined)
        return -1;
    std::vector<CThostFtdcDepthMarketDataField*> v;
    {
        ...
        for (auto& p : m_mdData) {
            auto& md = p.second;
            if (COMPARE_MEMBER_MATCH(pQryDepthMarketData, md, ExchangeID) && COMPARE_MEMBER_MATCH(pQryDepthMarketData, md, InstrumentID)) {
                v.emplace_back(&md);
            }
        }
    }
...
int CLocalTraderApi::ReqQrySettlementInfo(CThostFtdcQrySettlementInfoField* pQrySettlementInfo, int nRequestID)
{
    ...
    const std::string SELECT_NEWEST_SETTLEMENT_RECORD = "SELECT * FROM 'SettlementData' where BrokerID='" + m_brokerID
        + "' and InvestorID='" + m_userID
        + (!TradingDayForQuery.empty() ? ("' and TradingDay='" + TradingDayForQuery) : "")
    ...
    std::vector<CThostFtdcInvestorPositionDetailField*> v;
    {
        ...
        for (auto& o : m_positionData) {
            for (auto& t : o.second.posDetailData) {
                if (COMPARE_MEMBER_MATCH(pQryInvestorPositionDetail, t, ExchangeID) && COMPARE_MEMBER_MATCH(pQryInvestorPositionDetail, t, InstrumentID)) {
                    v.emplace_back(&t);
                }
            }
        }
...
int CLocalTraderApi::ReqQrySettlementInfoConfirm(CThostFtdcQrySettlementInfoConfirmField* pQrySettlementInfoConfirm, int nRequestID)
{
    ...
    const std::string SELECT_NEWEST_SETTLEMENT_RECORD = "SELECT ConfirmDay,ConfirmTime FROM 'SettlementData' where BrokerID='" + m_brokerID
