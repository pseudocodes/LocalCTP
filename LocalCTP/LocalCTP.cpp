...
{
    return rtnOrder.OrderStatus != THOST_FTDC_OST_PartTradedQueueing && rtnOrder.OrderStatus != THOST_FTDC_OST_NoTradeQueueing && rtnOrder.OrderStatus != THOST_FTDC_OST_Unknown && rtnOrder.OrderStatus != THOST_FTDC_OST_NotTouched;
}
    ...
    int tradedSize, std::vector<CThostFtdcTradeFieldWrapper>& Trades)
{
    ...
    for (size_t _index = 0; _index != SingleContracts.size(); ++_index) {
        ...
        } else // 若为第二腿
        {
            ...
            {
                Trade.OffsetFlag = (rtnOrder.CombOffsetFlag[0] == THOST_FTDC_OF_Open ? THOST_FTDC_OF_Close : THOST_FTDC_OF_Open);
            }
        }
        /// 价格
        ...
        /// 成交类型(普通成交/组合衍生成交)
        Trade.TradeType = SingleContracts.size() == 1 ? THOST_FTDC_TRDT_Common : THOST_FTDC_TRDT_CombinationDerived;
        /// 成交价来源
    ...
    const CThostFtdcInvestorPositionDetailField& posDetail)
{
    auto it = std::find_if(posDetailData.begin(), posDetailData.end(),
        [&](const CThostFtdcInvestorPositionDetailField& d) {
            return (strcmp(d.ExchangeID, posDetail.ExchangeID) == 0 && strcmp(d.OpenDate, posDetail.OpenDate) == 0 && strcmp(d.TradeID, posDetail.TradeID) == 0);
        });
...
CSettlementHandler::CSettlementHandler(CSqliteHandler& _sqlHandler)
    ...
    , m_tradingAccountUpdateFromPositionSql1(
          "UPDATE CThostFtdcTradingAccountField SET " ACCUMULATE_WITH_SAME_NAME(PositionProfit) ", " ACCUMULATE_WITH_SAME_NAME(CloseProfit) ", " ACCUMULATE_WITH_SAME_NAME(Commission) ", " ACCUMULATE_WITH_SAME_NAME(CashIn) ", " ACCUMULATE_WITH_DIFFERENT_NAME(CurrMargin, UseMargin) ", " ACCUMULATE_WITH_SAME_NAME(FrozenMargin) ", " ACCUMULATE_WITH_SAME_NAME(FrozenCommission) ", " ACCUMULATE_WITH_SAME_NAME(FrozenCash) ";")
    ...
    , m_timerThread([this]() {
        ...
        std::cout << "[LocalCTP] tradingDay is " << tradingDay << std::endl;
        ...
        while (m_running) {
            ...
            const int checkInterval = (CLocalTraderApi::m_runningMode == RUNNING_MODE::BACKTEST_MODE ? 10 : (60 * 2));
    ...
    const std::string& oldTradingDay)
{
    ...
    for (const auto& rowData : posDetailSqlValues) {
        ...
        if (!isOptions(itInstr->second.ProductClass)) {
            const double positionPrice(strcmp(posDetail.OpenDate, posDetail.TradingDay) == 0 ? posDetail.OpenPrice : posDetail.LastSettlementPrice);
            posDetail.PositionProfitByDate = (posDetail.Direction == THOST_FTDC_D_Buy ? 1.0 : -1.0) * (posDetail.SettlementPrice - positionPrice) * posDetail.Volume * itInstr->second.VolumeMultiple; // 更新持仓盈亏
            posDetail.PositionProfitByTrade = (posDetail.Direction == THOST_FTDC_D_Buy ? 1.0 : -1.0) * (posDetail.SettlementPrice - posDetail.OpenPrice) * posDetail.Volume * itInstr->second.VolumeMultiple; // 更新浮动盈亏
        }
        ...
        posDetail.Margin = posDetail.SettlementPrice * posDetail.Volume * itInstr->second.VolumeMultiple * posDetail.MarginRateByMoney + posDetail.Volume * posDetail.MarginRateByVolume; // 更新保证金
    ...
    for (const auto& rowData : posSqlValues) {
        ...
        if (!isOptions(itInstr->second.ProductClass)) {
            pos.PositionProfit = (pos.PosiDirection == THOST_FTDC_PD_Long ? 1.0 : -1.0) * (pos.SettlementPrice * pos.Position * itInstr->second.VolumeMultiple - pos.PositionCost); // 更新持仓盈亏
        }
        if (std::string(itInstr->second.ExpireDate) <= oldTradingDay) {
            // 模拟强制平仓, 对期权合约的持仓按结算价平仓(无交易手续费,不产生交易记录), 更新权利金收支(CashIn)
            if (isOptions(itInstr->second.ProductClass)) {
                pos.CashIn += (pos.PosiDirection == THOST_FTDC_PD_Long ? 1.0 : -1.0) * pos.SettlementPrice * pos.Position * itInstr->second.VolumeMultiple;
            }
            pos.Position = 0;
        }
        pos.PositionCost = pos.SettlementPrice * pos.Position * itInstr->second.VolumeMultiple; // 更新持仓成本
        pos.UseMargin = pos.PositionCost * pos.MarginRateByMoney + pos.Position * pos.MarginRateByVolume; // 更新持仓占用保证金
    ...
    for (const auto& yesterdayPos : yeterdayPosVec) {
        ...
        auto itTodayPos = std::find_if(todayPosVec.begin(), todayPosVec.end(),
            [&](const CThostFtdcInvestorPositionFieldWrapper& p) {
                return strcmp(p.data.BrokerID, pos.BrokerID) == 0 && strcmp(p.data.InvestorID, pos.InvestorID) == 0 && CLocalTraderApi::generatePositionKey(p.data) == relatedTodayPosKey;
            });
    ...
    for (auto& todayPos : todayPosVec) {
        todayPos.data.PositionDate = (CLocalTraderApi::isSpecialExchange(todayPos.data.ExchangeID) ? THOST_FTDC_PSD_History : THOST_FTDC_PSD_Today);
...
SET TradingDay = '"
        + newTradingDay + "', LastSettlementPrice = SettlementPrice, \
...
CloseVolume = 0, CloseAmount = 0;";
    ...
    const std::string positionUpdateSqlAfterSettlementSql1 = "DELETE FROM CThostFtdcInvestorPositionField WHERE Position = 0;";

    const std::string positionUpdateSqlAfterSettlementSql2 = "UPDATE CThostFtdcInvestorPositionField SET TradingDay = '" + newTradingDay
    ...
    const std::string& TradingDay)
{
    ...
    auto buildSettlementTradingAccount = [&]() {
        ...
        const std::string selectTradeSql = "SELECT SUM(CashIn) AS positiveCashIn FROM 'CThostFtdcTradeField' where BrokerID='"
    ...
    auto buildSettlementTrade = [&]() {
        CSqliteHandler::SQL_VALUES tradeSqlValues;
        const std::string selectTradeSql = "SELECT * FROM 'CThostFtdcTradeField' where BrokerID='"
            + brokerID + "' and InvestorID='" + userID
            + "' and TradingDay = '" + TradingDay
    ...
    auto buildSettlementCloseDetail = [&]() {
        ...
        const std::string selectCloseDetailSql = "SELECT * FROM 'CloseDetail' where BrokerID='"
            + brokerID + "' and InvestorID='" + userID
    ...
    auto buildSettlementPositionDetail = [&]() {
        ...
        const std::string selectPosDetailSql = "SELECT * FROM 'CThostFtdcInvestorPositionDetailField' where BrokerID='"
    ...
    auto buildSettlementPosition = [&]() {
        CSqliteHandler::SQL_VALUES posSqlValues;
        // 从数据库查找持仓数量大于0的持仓记录
        const std::string selectPosSql = "SELECT * FROM 'CThostFtdcInvestorPositionField' where BrokerID='"
            + brokerID + "' and InvestorID='"
        ...
        for (const auto& posInMergedPositionPair : mergedPositions) {
            ...
            // 买均价
            const double LongAvgPrice = (posInMergedPosition.LongPositon == 0 ? 0.0 : (posInMergedPosition.LongPositonCost / (1.0 * posInMergedPosition.LongPositon * ContractInfo.VolumeMultiple)));
            // 卖均价
            const double ShortAvgPrice = (posInMergedPosition.ShortPositon == 0 ? 0.0 : (posInMergedPosition.ShortPositonCost / (1.0 * posInMergedPosition.ShortPositon * ContractInfo.VolumeMultiple)));
    ...
    /// 结算单内容
    settlementData.SettlementContent = base64_encode(reinterpret_cast<const unsigned char*>(str_to_encode.c_str()),
